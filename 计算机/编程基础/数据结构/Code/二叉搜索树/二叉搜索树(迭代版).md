...
public class BinarySearchTree
{
    ...
    public List<int> PreOrder()
    {
        ...
        Stack<TreeNode> stack = new Stack<TreeNode>(); // 栈,存储待处理的节点
        stack.Push(_root); // 根节点入栈

        while (stack.Count > 0) // 还有待处理的节点
        {
            var node = stack.Pop();

            result.Add(node.Value); // 当前节点值加入返回列表中

            // 先压右子节点
            if (node.Right != null)
                stack.Push(node.Right);
            
            // 再压左子节点（栈先进后出，保证左子先处理）
            if (node.Left != null)
