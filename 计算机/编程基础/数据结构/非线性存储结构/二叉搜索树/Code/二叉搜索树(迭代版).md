...
public class BinarySearchTree
{
    ...
    {
        if (_root == null) // 根节点不存在,是一个空树,直接返回false
        {
            return false;
        }
    ...
    {
        List<int> result = new List<int>(); // 遍历结果
        ...
        {
            current = stack.Pop();
            result.Add(current.Value); // 先输出根节点

            // 由于是栈，先进后出，后进先出，这里先入栈右子节点，再入栈左子节点，出栈的时候，左就比右先出栈
    ...
    {
        List<int> result = new List<int>(); // 返回列表

        if (_root == null) // 根节点不存在,是一个空树
            return result;

        Stack<TreeNode> stack = new Stack<TreeNode>();
        TreeNode curr = _root; // 遍历指针

        // 循环条件：栈非空 或 当前节点非空（避免漏处理右子树）
    ...
    public List<int> PostOrder()
    {
        ...
        {
            current = stack.Pop();
            result.Add(current.Value); // 先添加当前节点值到结果中
            
            // 这里先入栈左，再入栈右，最终出栈就是右在左之前，即：中->右->左
            if (current.Left != null)
                stack.Push(current.Left);
        ...
        result.Reverse(); // 由于是中->右->左的顺序，这里需要翻转一下，就是左右中的顺序了
        return result;
    }
}
```