...
- **缓存机制：** `Resources`加载本身有内部缓存机制，当多次调用`Load`时，会先检查内存中是否已存在该资源的实例，如果内存已存在，则直接返回该实例，不会从硬盘重新加载
...
- **同名资源：** 由于`Unity`允许同名不同类型的资源，所以如果使用`Load(path)`这种没有传入筛选类型的方法，可能会加载到不是我们想要的资源